...
import {
  statusClass, statusLabel, formatTimestamp, calcTotal, formatPrice, showToast, distanceKm
...
const SIM_BOUNDS = { latMin: -17.88, latMax: -17.77, lonMin: 30.99, lonMax: 31.08 };
...
window.handleBikerLogin = function () {
  ...
  get(ref(db, `bikers/${bikerId}`)).then(snap => {
    ...
    document.getElementById('biker-name-sidebar').textContent = currentBiker.name;
...
window.showSection = function (name) {
  ...
  document.getElementById('section-title').textContent = {
    active: 'Active Delivery', map: 'My Map', history: 'Trip History'
  }[name];
...
function startGpsSimulation() {
  ...
  let lat = currentBiker.currentLat || randomBetween(SIM_BOUNDS.latMin, SIM_BOUNDS.latMax);
  let lon = currentBiker.currentLon || randomBetween(SIM_BOUNDS.lonMin, SIM_BOUNDS.lonMax);

  function nudge() {
    // Small random movement (≈100m per step)
    lat = clamp(lat + randomBetween(-0.001, 0.001), SIM_BOUNDS.latMin, SIM_BOUNDS.latMax);
    lon = clamp(lon + randomBetween(-0.001, 0.001), SIM_BOUNDS.lonMin, SIM_BOUNDS.lonMax);
    ...
    update(ref(db, `bikers/${currentBiker.bikerId}`), { currentLat: lat, currentLon: lon });
...
function listenForAssignment() {
  onValue(ref(db, `bikers/${currentBiker.bikerId}`), async snap => {
    ...
    sidebarStatus.className   = 'badge ' + (bikerData.status === 'AVAILABLE' ? 'badge-green' : 'badge-purple');

    if (bikerData.assignedOrderId && bikerData.status !== 'AVAILABLE') {
      // Load the order
      const orderSnap = await get(ref(db, `orders/${bikerData.assignedOrderId}`));
      ...
      if (order.status === 'ASSIGNED') {
        // Show full-screen job alert if not already accepted
        showJobAlert(order, bikerData.assignedOrderId);
      } else {
        // Job already accepted - show active job UI
        showActiveJobUI(order, bikerData.assignedOrderId, bikerData.deliveryPin);
      }
    }
  });
...
function showJobAlert(order, orderId) {
  const items   = (order.items || []).map(i => `• ${i.name} ×${i.qty} — ${formatPrice(i.price*i.qty)}`).join('<br>');
  const total   = formatPrice(calcTotal(order.items || []));
  ...
  document.getElementById('job-alert-items').innerHTML =
    `${items}<hr style="margin:8px 0;border:none;border-top:1px solid var(--gray-200)">
     <b>Total: ${total}</b><br>
     📍 ${order.gpsLat}, ${order.gpsLon}`;
...
window.acceptJob = async function () {
  ...
  // Update Firebase — admin and customer dashboards update instantly
...
window.declineJob = async function () {
  document.getElementById('job-alert').classList.add('hidden');
  // Reset biker assignment — admin will re-assign
  await update(ref(db, `bikers/${currentBiker.bikerId}`), {
    status: 'AVAILABLE', assignedOrderId: null, deliveryPin: null
  });
...
function showActiveJobUI(order, orderId, bikerPin) {
  ...
  // Init delivery map
  if (!bikerMap) {
    bikerMap = L.map('biker-map').setView([order.gpsLat || -17.8252, order.gpsLon || 31.0335], 14);
  ...
  if (order.gpsLat) {
    ...
    }).addTo(bikerMap).bindPopup(`📦 Deliver here<br>${order.customerName}<br>${order.deliveryAddress}`).openPopup();
  ...
  if (!myMarker) {
    ...
    }).addTo(bikerMap).bindPopup('You are here');
  }

  document.getElementById('delivery-status-pill').textContent = statusLabel(order.status);
  document.getElementById('delivery-status-pill').className   = 'badge ' + statusClass(order.status);
  ...
  document.getElementById('active-order-details').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
      ...
      <div>
        ...
        <p style="font-size:13px">${(order.items||[]).map(i=>`${i.name} ×${i.qty}`).join(', ')}</p>
      </div>
      <div>
        <p class="text-sm text-muted">Order Value</p>
        <p style="font-weight:700;color:var(--teal)">${formatPrice(calcTotal(order.items||[]))}</p>
      </div>
      ${order.deliveryNotes ? `<div style="grid-column:1/-1"><p class="text-sm text-muted">Notes</p><p style="font-size:13px">📝 ${order.deliveryNotes}</p></div>` : ''}
    </div>
  `;
}
...
function updateDeliveryStatusUI(status) {
  ...
  if (status === 'ASSIGNED') {
    ...
    hint.textContent    = 'Navigate to the delivery address. Tap "Arrived" when you reach the gate.';
  } else if (status === 'ARRIVED') {
    hint.textContent = 'Enter the box PIN on the outer keypad. Place the parcel and close the door.';
  } else if (status === 'DELIVERED') {
    doneBtn.disabled = false;
    hint.textContent = '✅ Parcel confirmed by locker sensor. Tap "Mark Delivered" to complete.';
  } else if (status === 'COLLECTED') {
    hint.textContent = '🎉 Delivery complete! Customer has collected the parcel.';
  }
...
window.confirmEnRoute = async function () {
  ...
  showToast('Status updated — En Route. Your box PIN is now visible.', 'success');
};
...
window.confirmDeliveryDone = async function () {
  ...
  setTimeout(() => {
    ...
    document.getElementById('delivery-status-pill').textContent = 'No active delivery';
    activeOrder = null; activeOrderId = null;
    if (myMarker) { bikerMap.removeLayer(myMarker); myMarker = null; }
    if (destMarker) { bikerMap.removeLayer(destMarker); destMarker = null; }
  }, 3000);
};
...
function renderBikerTimeline(currentStatus) {
  ...
  document.getElementById('biker-timeline').innerHTML = BIKER_STEPS.map((s, i) => `
...
function listenAllBikers() {
  onValue(ref(db, 'bikers'), snap => {
    ...
    Object.entries(bikers).forEach(([id, b]) => {
      ...
      const icon   = L.divIcon({ className:'', html: isSelf ? '🔵🛵' : '⚫🛵', iconSize:[40,24] });
      if (allBikerMarkers[id]) {
        ...
        allBikerMarkers[id] = L.marker([b.currentLat, b.currentLon], { icon })
          .addTo(allBikersMap)
          .bindPopup(`<b>${b.name}</b><br>${b.status}${isSelf ? ' (You)' : ''}`);
      }
    });
  });
}
...
function loadTripHistory() {
  onValue(ref(db, 'orders'), snap => {
    const orders = snap.val() || {};
    const myTrips = Object.entries(orders).filter(([, o]) =>
    ...
    tbody.innerHTML = myTrips.map(([id,o]) => `
      <tr>
...
window.logout = function () {
  ...
  currentBiker = null; activeOrder = null; activeOrderId = null;
};
