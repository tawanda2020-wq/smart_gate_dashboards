...
function listenBikers() {
  onValue(ref(db, "bikers"), (snap) => {
    ...
    const available = Object.values(allBikers).filter(
...
function listenLocker() {
  onValue(ref(db, "locker"), (snap) => {
    ...
    document.getElementById("locker-pill").textContent =
      "Locker: " + (locker.status || "IDLE");
    document.getElementById("locker-weight-pill").textContent =
      "Weight: " + Math.round(locker.weightGrams || 0) + "g";

    if (locker.breachAttempts > 0) {
      document.getElementById("breach-badge").classList.remove("hidden");
      addAlert(
        `🚨 ${locker.breachAttempts} failed PIN attempt(s) on the locker.`,
      );
    }
  });
    ...
    Object.entries(logs).forEach(([orderId, events]) => {
      Object.entries(events).forEach(([key, event]) => {
        if (key.startsWith("breach_")) {
          addAlert(
            `🚨 Breach on ${event.side} side — Order #${orderId?.slice(-6)} at ${formatTimestamp(event.timestamp)}`,
          );
        }
      });
    });
...
function renderOverview() {
  ...
  const pendingOrders = Object.entries(allOrders).filter(
    ([, o]) => o.status === "PENDING",
  );
  ...
  const recent = Object.entries(allOrders)
    ...
    .sort(
      ([, a], [, b]) =>
        (b.deliveredAt || b.timestamp) - (a.deliveredAt || a.timestamp),
    )
    .slice(0, 5);
...
function renderPendingOrders() {
  // Show both unassigned AND declined orders for re-assignment
  const pending = Object.entries(allOrders).filter(
    ([, o]) => o.status === "PENDING" || o.status === "DECLINED",
  );
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-header">
        <div>
          ...
          <span class="badge ${statusClass(o.status)}" style="margin-left:10px">${statusLabel(o.status)}</span>
...
window.openAssignModal = function (orderId) {
  ...
  const available = Object.entries(allBikers).filter(
    ([, b]) => b.status === "AVAILABLE",
  );
  select.innerHTML =
    '<option value="">— Choose available biker —</option>' +
    available
      .map(([id, b]) => `<option value="${id}">${b.name} (${b.phone})</option>`)
      .join("");
  ...
  select.onchange = () => {
    if (select.value) {
      ...
      document.getElementById("preview-biker-pin").textContent =
        generatedBikerPin;
      document.getElementById("preview-customer-pin").textContent =
        generatedCustomerPin;
...
window.confirmAssign = async function () {
  ...
  try {
    ...
    await update(ref(db, `orders/${pendingAssignOrderId}`), {
      status: "PENDING_ACCEPTANCE", // not ASSIGNED until biker accepts
    ...
    showToast(
      `✅ Assigned to ${biker.name}. PINs generated and pushed.`,
      "success",
    );
    addAlert(
      `📋 Order #${pendingAssignOrderId.slice(-6)} assigned to ${biker.name}.`,
    );
    closeAssignModal();
  } catch (err) {
...
function renderBikersTable() {
  ...
  if (!bikers.length) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="text-center text-muted">No bikers registered.</td></tr>';
    return;
  }
  tbody.innerHTML = bikers
    .map(
      (b) => `
    <tr>
...
function updateBikerMapMarkers() {
  ...
  Object.entries(allBikers).forEach(([id, b]) => {
    ...
    bikerMarkers[id] = L.marker([b.currentLat, b.currentLon], {
      icon: L.divIcon({
        className: "",
        html: b.status === "AVAILABLE" ? "🟢🛵" : "🔴🛵",
        iconSize: [40, 24],
      }),
    })
      .addTo(adminMap)
      .bindPopup(`<b>${b.name}</b><br>${b.status}<br>${b.phone}`);
  });
  ...
  Object.entries(allOrders)
    ...
    .forEach(([id, o]) => {
      ...
      L.marker([o.gpsLat, o.gpsLon], {
        ...
        .addTo(adminMap)
        .bindPopup(
          `<b>${o.customerName}</b><br>#${id.slice(-6)}<br>${o.deliveryAddress}`,
        );
    });
}
...
function renderHistoryRows(rows) {
  ...
  if (!rows.length) {
    tbody.innerHTML =
      '<tr><td colspan="6" class="text-center text-muted">No completed deliveries yet.</td></tr>';
    return;
  }
